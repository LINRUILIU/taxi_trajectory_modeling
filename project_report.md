...
仓库发布说明：
- `release/full-artifacts`：完整归档版，保留路网、核心数据、定版预测、关键中间结果与报告引用产物；其中 `map`、`*.pkl`、`*.joblib`、关键 `*.csv` 等大文件通过 Git LFS 管理。
- `main`：主发布版，保留代码、配置、文档、报告、路网与最小复现实验数据，删除大部分已生成运行结果，便于按 README 从干净状态复现。
- `release/code-lite`：轻量阅读版，仅保留核心代码、配置与文档，不附带数据、路网和模型结果，用于快速审阅项目结构。
